...
def analyze_file_completeness(csv_file_path, building_field_name):
    ...
    climate_zones = {"All", "1A", "2A", "2B", "3A", "3B", "3C", "4A", "4B", "4C", "5A", "5B", "6A", "6B", "7", "8"}

    year_built_ranges = {"All", "Before 1946", "1946-1959", "1960-1979", "1980-1999", "2000-2009", "2010 and after"}

    weekly_hours_ranges = {"All", "Fewer than 40", "40-48", "48.01 - 60", "60.01 - 84", "84.01-167", "Open Continuously"}
    ...
    total_possible = len(building_types) * len(climate_zones) * len(year_built_ranges) * len(weekly_hours_ranges) * len(gfa_ranges)
    ...
    print(
        f"EUI completeness of possible combinations: {completeness_of_possible:.1f}% ({combinations_with_eui_count:,}/{total_possible:,})"
    )
    print(
        f"EUI completeness of actual combinations: {completeness_of_actual:.1f}% ({combinations_with_eui_count:,}/{actual_combinations_count:,})"
    )
...
def main():
    ...
    if len(results) == 2:
        ...
        print(
            f"{'EUI completeness of possible (%)':<40} {results[0]['eui_completeness_of_possible_percent']:<14.1f} {results[1]['eui_completeness_of_possible_percent']:<14.1f}"
        )
        print(
            f"{'EUI completeness of actual (%)':<40} {results[0]['eui_completeness_of_actual_percent']:<14.1f} {results[1]['eui_completeness_of_actual_percent']:<14.1f}"
        )
        print(
            f"{'Row completeness (%)':<40} {results[0]['row_completeness_percent']:<14.1f} {results[1]['row_completeness_percent']:<14.1f}"
        )
