...
def analyze_field_distributions_and_impact():
    ...
    for field in fields:
        ...
        print(f"Most common values with EUI success rates:")
        
        # Sort by total count (most common first)
        sorted_values = sorted(value_success_rates.items(), 
    ...
    for record in eui_data:
        combo = (record['building_type'], record['climate_zone'], 
                record['year_built'], record['weekly_hours'], record['gfa'])
    ...
    for record in test_sample:
        bt, cz, yb, wh, gfa = (record['building_type'], record['climate_zone'], 
                              record['year_built'], record['weekly_hours'], record['gfa'])
    ...
    success_rate = ((tier_hits['exact'] + tier_hits['relax_hours'] + 
                    tier_hits['relax_hours_year'] + tier_hits['relax_hours_year_gfa']) / total_tests * 100)
    ...
    relaxation_ranking = sorted(relaxation_impact.items(), 
                               key=lambda x: x[1]['relaxation_potential'], reverse=True)
    ...
    print(f"\nKey Insights:")
    print(f"1. Your instinct is CORRECT: building_type and climate_zone should be preserved")
    print(f"2. Current relaxation order is optimal: weekly_hours → year_built → gfa")
